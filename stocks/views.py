...
from .my_pachong import get_stock_all_a_dc,get_stock_all_ggt_dc,get_all_etf_dc
...
def get_stock_information(code):
    ...
    realtime_stock = get_stock_all_a_dc()
    realtime_bond = ak.bond_zh_hs_cov_spot()  # 可转债行情
    # realtime_etf = ak.fund_etf_spot_em()  # etf行情
    realtime_etf = get_all_etf_dc()  # etf行情
    # realtime_hk_stock = ak.stock_hk_spot_em()  # 港股行情
    realtime_hk_stock = get_stock_all_ggt_dc()

    if len(code) == 6:
        if code[0] in ['6','4','8','0','3']:   # 识别 股票
            stock_df = realtime_stock[realtime_stock['代码']==code]
            name = stock_df['名称'].values[0]
            close = Decimal(stock_df['最新价'].values[0])
            ratio = stock_df['涨跌幅'].values[0]
            ...
            name = stock_df['name'].values[0]
            close = Decimal(stock_df['trade'].values[0])
            ratio = stock_df['changepercent'].values[0]
        else:                   # etf
        ...
        stock_df = realtime_hk_stock[realtime_hk_stock['代码'] == code]
        name = stock_df['名称'].values[0]
        close = Decimal(stock_df['最新价'].values[0])
        ratio = stock_df['涨跌幅'].values[0]
    #
    ...
    dict = {'name': name, 'close': close, 'ratio': ratio, }
    return dict
...
def refresh_stock():
    ...
    realtime_etf = get_all_etf_dc()  # etf行情
    # realtime_hk_stock = ak.stock_hk_spot_em()  # 港股行情
    realtime_hk_stock = get_stock_all_ggt_dc()  # 港股行情
...
def profit(request):
    ...
    ax.set_ylabel('Ratio')
    ax.legend()
    ax.set_title('My profit Vs Hs300')
    # 将图形保存到内存中的字节流中
    buffer = BytesIO()
    ...
    # 传递图像到模板
    return render(request, 'profit.html', {'graphic1': graphic1,'graphic2': graphic2})
